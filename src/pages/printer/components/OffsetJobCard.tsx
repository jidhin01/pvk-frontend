...
import { Settings, Play, CheckCircle, AlertTriangle, Calendar, Printer } from 'lucide-react';
...
export default function OffsetJobCard({ job, onStatusChange, onReject }: OffsetJobCardProps) {
    ...
    return (
        <Card className="flex flex-col shadow-md hover:shadow-xl transition-all border-t-8 border-t-slate-800 dark:border-t-slate-400 bg-slate-50/50 dark:bg-slate-900/50 h-full">
            ...
            <CardFooter className="pt-4 pb-6 bg-slate-100/50 dark:bg-slate-950/30 border-t border-slate-200 dark:border-slate-800">
                <div className="flex w-full gap-3">
                    {job.status === 'ready_for_print' ? (
                        ...
                        <Button className="flex-1 h-12 bg-green-600 hover:bg-green-700 shadow-lg shadow-green-600/20" onClick={() => onStatusChange(job.id, 'completed')}>
                            <CheckCircle className="mr-2 h-5 w-5" /> Mark Run Complete
                        </Button>
                    )}
